<section id="{{ $posts->first()->postCategory->slug }}">
  <div class="bg-primary-color dark:bg-black-color py-60px md:py-20 lg:py-30 overflow-x-hidden">
    <div class="container">
      <div class="">
        <div class="flex items-center mb-30px sm:mb-10">
          <ul id="tabs"
            class="max-w-full mx-auto inline-flex items-center justify-center bg-primary-color rounded-full p-5px relative z-0">
            @foreach($posts as $index => $post)
            <li class="{{ $loop->first ? 'active' : '' }}">
              <a class="text-sm sm:text-size-15 font-bold px-15px sm:px-25px py-10px sm:py-11px text-white-color bg-transparent rounded-full"
                href="#{{ Str::slug($post->title) }}">
                {{ $post->title }}
              </a>
            </li>
            @endforeach
        ...
        <div id="tab-contents">
          @foreach($posts as $index => $post)
          <div id="{{ Str::slug($post->title) }}"
            class="tab-contents tab-pane fade {{ $loop->first ? 'show active' : '' }}">
            <div class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-5 md:gap-30px">
              <div
                ...
                <div
                  class="absolute m-4 right-0 left-0 bottom-0 bg-gradient-secondary bg-200 rounded-15px p-3 inline-flex justify-center items-center leading-1">
                  <a href="/"
                    class="text-xl text-white-color leading-1 transition-all duration-300 inline-flex justify-center items-center hover:text-primary-color">
                    اشتراك
              ...
              {{-- post image  --}}
              <div
                class="overflow-hidden col-span-2 px-15px md:px-25px lg:px-10 bg-cream-light-color dark:bg-primary-color-light">
