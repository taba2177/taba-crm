    ...
    <div class="container">
      ...
      <div>
        <ul class="tj-service-5-accordion-list accordion-list">
          ...
          <li class="bg-white dark:bg-dark-color px-15px lg:px-25px xl:p-30px py-25px lg:py-30px border border-body-color dark:border-bg-color-2 cursor-pointer mb-30px last:mb-0 rounded-15px group/main wow fadeInUp" data-wow-delay=".1s">
            <h3 class="tj-service-5-accordion-list-title text-xl sm:text-size-22 md:text-3xl font-bold leading-1.2 md:leading-1.2 tracking-0.02em text-seondary-color dark:text-white-color transition-all duration-300 ease-in relative before:content-['\f123'] before:font-flaticon before:absolute before:text-xl before:right-0 rtl:before:left-0 rtl:before:right-auto before:top-0 before:opacity-1 before:visible before:rotate-0 before:transition-all before:duration-300 before:ease-in before:font-normal group-hover/main:before:text-primary-color group-hover/main:before:rotate-90">
              <span class="text-primary-color mr-1">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}.</span> {{$item->title }}
            </h3>
            <div class="tj-service-5-accordion-wrap flex justify-between items-center flex-wrap gap-y-50px">
              <div class="tj-service-5-accordion-list-content w-full max-w-504px lg:max-w-400px xl:max-w-430px ml-8 md:ml-50px mt-30px md:mt-35px lg:mt-50px transition-all duration-300 ease-in">
              ...
              <div class="tj-service-5-accordion-list-image w-full lg:w-auto">
                <div class="tj-service-5-accordion-thumb w-full max-w-full lg:max-w-400px xl:max-w-540px 2xl:max-w-615px">
                  <img src="{{ asset($item->image?->url ?? $item->getRandomImage()) }}" alt="{{$item->title }}" class="service-image w-full h-full object-cover rounded-15px">
                </div>
              </div>
            </div>
          </li>
