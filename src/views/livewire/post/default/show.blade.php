            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 lg:gap-12">
                ...
                <aside class="hidden lg:block lg:col-span-1">
                    <div class="sticky top-24 space-y-6">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ $post->postCategory->name }}
                            {{ __('related') }}</h3>
                        @foreach ($relatedPosts->take(3) as $relatedPost) {{-- Show a few posts in sidebar --}}
                        <a href="{{ $relatedPost->url }}"
                            class="group flex items-start gap-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                            <img src="{{ $relatedPost->image?->url ?? $relatedPost->getRandomImage() }}"
                                alt="{{ $relatedPost->title }}" class="w-24 h-24 object-cover rounded-md flex-shrink-0">
                            <div class="flex-grow">
                                <h4
                                    class="font-bold text-base leading-tight text-gray-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
            ...
            <div class="container">
                <div
                    ...
                    <div class="md:ml-auto flex-shrink-0 m-auto">
                        <a href="#"
                            class="inline-block px-6 py-3 text-base font-semibold text-white bg-primary-600 rounded-lg hover:bg-primary-700 transition-colors shadow">{{__('Contact us')}}</a>
                    </div>
                </div>
            </div>
        </div>
