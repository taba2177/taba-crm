<div>
  ...
  <x-container>
    ...
    <div class="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div>
        <a class="*:transition group" href="{{ $post->url }}" wire:navigate title="{{ $post->title }}">
          <div class="w-full h-48 mb-2 overflow-hidden border rounded-lg group-hover:brightness-90">
            @if ($post->image)
            <img class="object-cover w-full h-full" src="{{ $post->image?->url ?? $post->getRandomImage() }}" alt="{{ $post->image->alt ?? $post->title }}" />
            @else
          ...
          <div class="meta-box">
            <a href="#">{{ $post->category }}</a> <span>/</span> {{ $post->date }}
          </div>
