<div>
  <div class="main-wrapper">
    <!-- breadcrumb -->
    <section>
      <div class="hero-breadcurmb pt-150px md:pt-40 lg:pt-200px pb-50px md:pb-60px lg:b-100px bg-cover bg-center bg-no-repeat relative z-1 after:absolute after:top-0 after:left-0 after:rtl:left-auto rtl:after:right-0 after:w-full after:h-full after:bg-primary-color-light after:-z-1 after:opacity-70">
          ...
          <div class="flex flex-col items-center">
            ...
            <ul class="nav flex items-center gap-x-10px">
              <li class="nav_item group relative">
                <a href="{{ url('/') }}" class="font-medium text-white-color capitalize relative z-0 after:w-0 after:h-1px after:bg-white-color after:absolute after:left-0 after:rtl:left-auto after:rtl:right-0 after:bottom-0 after:transition-all after:duration-500 group-hover:after:w-full">
                  الرئيسية
                </a>
              </li>
              <li class="nav_item group relative">
                <a href="{{ route('posts') }}" class="font-medium text-white-color capitalize relative z-0 after:w-0 after:h-1px after:bg-white-color after:absolute after:left-0 after:rtl:left-auto after:rtl:right-0 after:bottom-0 after:transition-all after:duration-500 group-hover:after:w-full">
                  ...
                  <div class="relative z-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 gap-x-50px items-start px-15px md:px-25px lg:px-10 mb-10 md:mb-50px">
                      <div>
                        ...
                        @if (auth()->check())
                        <div>
                          <a href="{{ $post->editUrl }}" title="تعديل المقال" class="text-size-15 font-bold text-white-color capitalize py-17px px-35px bg-200 bg-gradient-secondary hover:bg-[-100%] rounded-full leading-1 transition-all duration-300 whitespace-nowrap group/nested">
                            تعديل المقال
                            <i class="fal fa-arrow-left ml-10px rtl:mr-10px -rotate-45 group-hover/nested:rotate-0 transition-all duration-300"></i>
                          </a>
                        </div>
                        @endif
                      </div>
                    ...
                    <div class="mb-10 md:mb-50px">
                      <div class="portfolio_gallery owl-carousel">
                        ...
                        <div class="gallery_item">
                          <a href="{{ route('curtains', ['category' => $relatedProduct->product_category->url, 'product' => $relatedProduct->url]) }}" title="{{$relatedProduct->name}} ستاير صن رول - ">
                            @if (!empty($relatedProduct->image) && is_array($relatedProduct->image))
                            <img loading="lazy" src="{{ asset('storage/' . $relatedProduct->image[0]) }}" alt="{{ $relatedProduct->name ?? 'Product image' }}" style="aspect-ratio: 1/1;">
                            @elseif (!empty($relatedProduct->image))
                            <img loading="lazy" src="{{ asset('storage/' . $relatedProduct->image) }}" alt="{{ $relatedProduct->name ?? 'Product image' }}" style="aspect-ratio: 1/1;">
                            @else
                            <img src="{{ asset('/assets/images/floating_logo.png') }}" alt="Default Image" style="aspect-ratio: 1/1;">
                            @endif
                          </a>
                        </div>
                        @endforeach
                    ...
                    <div class="widget-wrap text-center bg-light-theme rounded py-5 px-15px md:px-25px lg:px-10 mb-10 md:mb-50px">
                      ...
                      <a href="{{ url("tel:966501253111") }}" class="text-size-15 font-bold text-white-color capitalize py-17px px-35px bg-200 bg-gradient-secondary hover:bg-[-100%] rounded-full leading-1 transition-all duration-300 whitespace-nowrap inline-block" title="اتصل على رقم التواصل 966501253111">
                        تواصل معنا
                        <i class="fal fa-arrow-left ml-10px rtl:mr-10px -rotate-45 group-hover/nested:rotate-0 transition-all duration-300"></i>
