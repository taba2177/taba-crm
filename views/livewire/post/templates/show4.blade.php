<x-layouts.main :title="$post->title">
  <div>
    <crm::x-banner :title="$post->postCategory->description" :post="$post"
      backgroundImage="{{  $post->image?->url ?? $post->getRandomImage()}}">
                  ...
                  <div class="relative z-10">
                    <div
                      ...
                      <div>
                        ...
                        @if (auth()->check())
                        <x-button href="{{ $post->editUrl }}" label="تعديل " icon="fa-arrow-left" />
                        @endif
                      </div>


                      <x-portfolio.meta-grid>
                        <x-portfolio.meta-item label="التصنيف"
                          value="{{ $post->tags->isNotEmpty() ? $post->tags->first()->name : $post->postCategory->name.' / '.$post->metadata[0]['value'] ?? 'عام' }}" />

                        <x-portfolio.meta-item label="المنفذ" value="{{ $post->user->name }}" />
                        <x-portfolio.meta-item label="تاريخ العمل"
                          value="{{ $post->published_at->translatedFormat('j F، Y', 'ar') }}" />
                    ...
                    <div class="px-15px md:px-25px lg:px-10">
                      <x-portfolio.section>
                        <div class="prose max-w-none text-primary-color-light dark:text-white-color">
                    ...
                    <div class="px-15px md:px-25px lg:px-10 mb-10 md:mb-50px">
                      @include('components.homepage.' . $post->homepage_section_component, ['content' =>
                    ...
                    <x-portfolio.gallery class="mb-10 md:mb-50px">
                      ...
                      <x-portfolio.gallery-item :title="$post->title" :url="$post->url"
                        src="{{ $post->image?->url ?? $post->getRandomImage() }}" alt="{{ $post->title }}" />
                      @endforeach
                    </x-portfolio.gallery>
                    @endif
