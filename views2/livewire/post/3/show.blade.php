<x-layouts.main :title="$post->title">
  <div>
    <crm::x-banner :title="$post->postCategory->description" :post="$post" backgroundImage="{{  $post->image?->url ?? $post->getRandomImage()}}">
                  ...
                  <div class="relative z-10">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 gap-x-50px items-start px-15px md:px-25px lg:px-10 mb-10 md:mb-50px">
                      ...
                      <x-portfolio.meta-grid>
                        <x-portfolio.meta-item label="التصنيف" value="{{ $post->tags->isNotEmpty() ? $post->tags->first()->name : $post->postCategory->name.' / '.$post->metadata[0]['value'] ?? 'عام' }}" />

                        <x-portfolio.meta-item label="المنفذ" value="{{ $post->user->name }}" />
                        <x-portfolio.meta-item label="تاريخ العمل" value="{{ $post->published_at->translatedFormat('j F، Y', 'ar') }}" />
                    ...
                    <x-portfolio.gallery class="mb-10 md:mb-50px">
                      ...
                      <x-portfolio.gallery-item :title="$post->title" :url="$post->url" src="{{ $post->image?->url ?? $post->getRandomImage() }}" alt="{{ $post->title }}" />
                      @endforeach
                    </x-portfolio.gallery>
                    @endif
